...
public class HasItemRequirement extends Requirement {
    ...
    boolean estimate(@NotNull final Player wiver, @Nullable final MenuPlaceholderContext menuPlaceholderContext) {
        String material = MenuSettingsAddon.setPlaceholders(wiver, wrapper.getIcon() != null ? wrapper.getIcon() : "", menuPlaceholderContext).toUpperCase(Locale.ROOT);
        ItemChecks itemChecks = this.wrapper.getItemChecks();
        if (itemChecks == null) return false;

        ItemStack itemStack = CreateItemStack.of(material).makeItemStack();
        ...
        for (ItemStack inventoryItem : playerInventory) {
            if (checkIfIsRequiredItem(menuPlaceholderContext, inventoryItem, itemChecks, wiver, itemStack.getType()))
    ...
    private boolean checkIfIsRequiredItem(@Nullable final MenuPlaceholderContext menuPlaceholderContext, ItemStack itemToCheck, ItemChecks itemChecks, Player wiver, Material material) {
        ...
        if (itemMetaOnInventoryItem != null) {
            ...
            if (this.wrapper.getDisplayname() != null) {
                if (itemMetaOnInventoryItem.hasDisplayName()) {
                    String itemToMatch = formatColors(MenuSettingsAddon.setPlaceholders(wiver, this.wrapper.getDisplayname(), menuPlaceholderContext));
                    String itemInInventory = formatColors(MenuSettingsAddon.setPlaceholders(wiver, itemMetaOnInventoryItem.getDisplayName(), menuPlaceholderContext));
